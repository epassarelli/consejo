<div>
    ...
    <div class="container mt-4">
        <div class="row">
            ...
            <div class="col-md-4 text-right">
                <button class="btn btn-secondary" wire:click="volver" data-target="#itemModal"><i
                        class="fas fa-arrow-circle-left  mr-2" style="color: white;"></i>Volver</button>
                @if ($esAdmin && in_array($sesion->estado, [1, 4]))
                    <button class="btn btn-success" wire:click="openModal" data-target="#itemModal"><i
                            class="fas fa-plus-circle mr-2" style="color: white;"></i>Agregar Item</button>
                    <button class="btn btn-warning" wire:click="createVotacion" data-target="#itemModalVotacion"><i
                            class="fas fa-plus-circle mr-2" style="color: white;"></i>Agregar Votación</button>
                @endif
            </div>
        ...
        <div class="row">
                    ...
                    <div class="mt-1">
                        @if (!empty($votacionActiva) && $votacionActiva->id == $votacionId && $votacionEstado == 3)
                            <div class="ribbon-wrapper ribbon-lg">
                                <div
                                    class="ribbon {{ !empty($votacionActiva->resultado) ? 'bg-success' : ($votacionActiva->resultado === null ? 'bg-secondary' : 'bg-danger') }}">
                        ...
                        <div class="col-md-12 input-group">
                            <div class="col-4 input-group-prepend">
                                ...
                                <input type="text" class="form-control" name="VotTitulo" wire:model="votacionTitulo"
                                    @if (!$esAdmin || $votacionEstado != 1) disabled @endif>
                            </div>
                            <div class="col-3 input-group-prepend">
                                ...
                                <select class="form-control" name="comision" id="comision"
                                    wire:model="votacionAceptacion" @if (!$esAdmin || $votacionEstado != 1) disabled @endif>
                            ...
                            @if ($esAdmin)
                                <div class="col-5 text-right">
                                    @if ((empty($votacionActiva) || $votacionActiva->id != $votacionId) && $votacionEstado != 3)
                                        <button type="button" wire:click="removeVotacion({{ $votacionId }})"
                                            class="btn btn-danger">Eliminar</button>&nbsp;
                                        <button wire:click="updateVotacion"
                                            class="btn btn-primary">Actualizar</button>&nbsp;
                                    @endif
                                    @if (!empty($votacionActiva) && $votacionActiva->id == $votacionId && $votacionEstado == 2)
                                        <button wire:click="enableVotacion({{ $votacionId }},3)"
                                            class="btn btn-secondary">Cerrar Votación</button>&nbsp;
                                        <button wire:click="enableVotacion({{ $votacionId }},1)"
                                            class="btn btn-warning">Pausar Votación</button>
                                    @elseif(
                                        $sesion->ordenDia->id_estado == 6 &&
                                            (empty($votacionActiva) || $votacionActiva->estado != 2) &&
                                            $sesion->ordenDia->votacionesActivas()->count() == 0)
                                        <button wire:click="enableVotacion({{ $votacionId }},2)"
                                            class="btn btn-warning">Habilitar Votación</button>
                                    @endif
                                </div>
                            @endif
                        ...
                        <div class="row mt-3">
                            ...
                            <div class="col-lg-2 col-6">
                                <div class="small-box bg-secundary">
                                    <div class="inner">
                                        <h3>{{ $sesion->votantes->count() - $votacionActiva->participantes_count }}
                                        </h3>
                                        <p>Faltan</p>
                                    </div>
                                    <div class="icon">
                            ...
                            @if (
                                !empty($votacionActiva) &&
                                    $votacionActiva->estado == 2 &&
                                    $sesion->votantes()->where('users.id', Auth::user()->id)->exists())
                                <div class="col-lg-2 col-6">
                                    <div class="small-box bg-secundary">
                                        <div class="inner">
                                            <p>Voto</p>
                                            @if (!$votacionActiva->participantes()->where('users.id', Auth::user()->id)->exists())
                                                <button wire:click="setVoto(true)" class="btn btn-success"
                                                    title="Afirmativo"><i class="fa fa-thumbs-up"></i></button>&nbsp;
                                                <button wire:click="setVoto(false)" class="btn btn-danger"
                                                    title="Negativo"><i class="fa fa-thumbs-down"></i></button>
                                                <button wire:click="setVoto(null)" class="btn btn-info"
                                                    title="Absenerse"><i class="fa fa-yin-yang"></i></button>
                                            @else
                                                <button wire:click="removeVoto({{ $votacionId }})"
                                                    class="btn btn-warning">Borrar Voto</button>
                                            @endif
                                        </div>
                                    </div>
                                </div>
            ...
            <div class="mt-2 col-md-12 text-left btn-group btn-group-toggle" style="overflow-x: auto">
                @foreach ($votaciones as $votacion)
                    <label
                        ...
                        style="text-decoration: {{ $votacion->estado == 3 &&$votacion->resultado == null &&$sesion->votantes()->where('users.id', Auth::user()->id)->exists() &&!$votacion->participantes()->where('users.id', Auth::user()->id)->exists()? 'line-through': 'none' }}"><input
                            type="radio" name="votacion" id="voto_{{ $votacion->id }}" autocomplete="off"
                            wire:click="activeVotacion({{ $votacion->id }})"
                            @if (!$esAdmin && !in_array($sesion->ordenDia->id_estado, [2, 3, 5])) disabled @endif /> {{ $votacion->titulo }}</label>
                @endforeach
            </div>
                    ...
                    <table class="table table-hover table-bordered mt-3">
                        <thead>
                            <tr>
                                ...
                                <th wire:click="sortBy('items_temario.orden')" class="text-center">ORDEN
                                    @if ($sortColumn == 'items_temario.orden')
                                        @if ($sortDirection == 'asc')
                                            <i class="fas fa-sort-up"></i>
                                        @else
                                            <i class="fas fa-sort-down"></i>
                            ...
                            <tr>
                                ...
                                <th class="text-center">
                                    <div class="row">
                                        <div class="col-sm-12">
                                            <input wire:model="searchByComision" type="search"
            ...
            <th class="text-center">
                <div class="row">
                    <div class="col-sm-12">
                        <input wire:model="searchByFaculty" type="search" placeholder="Buscar por facultad"
                            class="form-control form-control-sm">
            </th>
        </div>
    </div>
    ...
    <th class="text-center">
        <div class="row">
            <div class="col-sm-12">
                <input wire:model="searchByExp" type="search" placeholder="Buscar por expediente"
                    class="form-control form-control-sm">
    </th>
</div>
</div>
...
<th class="text-center">
    <div class="row">
        <div class="col-sm-12">
            <input wire:model="searchByResolution" type="search" placeholder="Buscar por resolucion"
                class="form-control form-control-sm">
</th>
<th class="text-center">
    <div class="row">
        <div class="col-sm-12">
            <input wire:model="searchByOrden" type="search" placeholder="Buscar por orden"
                class="form-control form-control-sm">
</th>
</div>
</div>
...
<tbody>
    @foreach ($items as $item)
        <tr>
            ...
            <td
                class="{{ ($itemEnVotacionActiva = !empty($votacionId) && $item->id_votacion == $votacionId) ? 'bg-warning' : '' }}">
                {{ $item->tema->titulo }}</td>
            ...
            <td class="{{ $itemEnVotacionActiva ? 'bg-warning' : '' }} text-center">{{ $item->tipo }} Nro
                {{ $item->numero }}</td>
            <td class="{{ $itemEnVotacionActiva ? 'bg-warning' : '' }} text-center">{{ $item->resolucion }}</td>
            <td class="{{ $itemEnVotacionActiva ? 'bg-warning' : '' }} text-center">{{ $item->orden }}</td>
            <td class="{{ $itemEnVotacionActiva ? 'bg-warning' : '' }} p-1 text-center">
                <button wire:click="openEditModal({{ $item->id }}, true)" class="btn btn-sm btn-secondary"
                    title="Ver"><i class="fa fa-eye"></i></button>
                <button wire:click="openAttachModal({{ $item->id }})" class="btn btn-sm btn-primary"
                    title="atach"><i class="fa fa-file-pdf"></i></button>
                @if ($esAdmin && in_array($sesion->estado, [1, 4]))
                    <button wire:click="openEditModal({{ $item->id }}, false)" class="btn btn-sm btn-primary"
                        title="Editar"><i class="fa fa-edit"></i></button>
                    <button wire:click="$emit('alertDelete',{{ $item->id }})" class="btn btn-sm btn-danger"
                ...
                @if (
                    ...
                    @if (empty($item->id_votacion))
                        <button wire:click="addVotacion({{ $item->id }}, {{ $votacionId }}, true)"
                            class="btn btn-sm btn-warning" title="Agregar"
                            @if ($item->id_votacion) disabled @endif><i class="fa fa-plus"></i></button>
                    @elseif($item->id_votacion == $votacionId)
                        <button wire:click="addVotacion({{ $item->id }}, {{ $votacionId }}, false)"
                            class="btn btn-sm bg-white" title="Quitar"><i class="fa fa-minus"></i></button>
                    @endif
                @endif
            </td>
        </tr>
...
<!-- Modal -->
@if ($showActionModal)
    <div wire:ignore.self class="modal fade show" id="itemModal" tabindex="-1" role="dialog"
        aria-labelledby="itemModalLabel" aria-hidden="true" style="display: {{ $showActionModal ? 'block' : '' }}">
                                ...
                                <div class="input-group mb-2 mr-sm-2">
                                    ...
                                    <div class="col-4 input-group-prepend">
                                        ...
                                        <input type="text" class="form-control" name="numero" id="numero"
                                            wire:model="numero" @if ($readonly) disabled @endif>
                                        @error('name')
                                            <span class="text-danger">{{ $message }}</span>
                                        @enderror
                                    </div>
                            ...
                            <div class="row">
                                <div class="input-group mb-2 mr-sm-2">
                                    <div class="col-12 input-group-prepend">
                                        ...
                                        <textarea class="form-control" name="resumen" id="resumen" cols="30" rows="3" wire:model="resumen"
                                            @if ($readonly) disabled @endif></textarea>
...
@if ($showAttachModal)
    ...
    <div wire:ignore.self class="modal fade show" id="itemModal2" tabindex="-1" role="dialog"
        aria-labelledby="itemModalLabel" aria-hidden="true" style="display: {{ $showAttachModal ? 'block' : '' }}">
        <div class="modal-dialog modal-lg" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h3 class="well">Documentos adjuntos</h3>
                    <button type="button" class="close" wire:click="closeAttachModal" data-dismiss="modal"
                        aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                        ...
                        <tbody>
                            @foreach ($archivos as $arch)
                                <tr>
                                    <td>{{ $arch->title }}</td>
                                    <td>
                                        <a href="{{ 'descargar-pdf?path=' . $arch->path }}"
                                            download>{{ $arch->name }}</a>
                                    </td>
                                    <td class="text-center">
                                        <button wire:click="deleteAdj({{ $arch->id }})"
                        ...
                        <div class="row">
                            <div class="input-group mb-2 mr-sm-2">
                                <div class="col-12 input-group-prepend">
                                    ...
                                    <button type="button" wire:click="guardarArchivos"
                                        class="btn btn-sm btn-primary" accept="application/pdf"
                                        wire:loading.attr="disabled"><i class="fas fa-upload"></i></button>
                                </div>
